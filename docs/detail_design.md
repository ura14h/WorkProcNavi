...
Renderer はリンク hover をイベント委譲で捕捉し、約 0.5 秒後に URI をプレビュー表示する。
URI プレビューは、リンク上からカーソルが外れた時点、ウィンドウ外へ出た時点、またはリンクを開いた時点で消去する。
...
- 解析不能な URL、許可プロトコル以外の URL、親ディレクトリまで解決できない `file://` URL は開かずエラーとして扱う

`file://` の動作は以下とする。

1. 対象がディレクトリの場合、標準ファイルエクスプローラまたは Finder でそのディレクトリを開く
2. 対象がファイルの場合、ファイルを直接開かず、親ディレクトリを開いて対象ファイルを選択または表示する
3. 対象ファイル自体が見つからず親ディレクトリが存在する場合は、ファイルが見つからないことを表示し、親ディレクトリだけを開くか利用者へ確認する
4. 利用者が続行を選んだ場合は親ディレクトリを開く
5. OS がファイル選択表示を提供できない場合は、親ディレクトリを開く
...
`fileItem` は実在するファイル本体ではなく親ディレクトリ内で対象ファイルを表示したことを表す。
対象ファイルが見つからず、確認後に親ディレクトリのみを開いた場合は `fileDirectory` を返す。
...
リンクにマウスカーソルを合わせた場合は、約 0.5 秒後に URI を小さなプレビューとして表示する。
このプレビューはクリック可否の判断材料であり、リンクの安全性判定自体は Main Process 側の `openManualLink` で行う。
カーソルがリンク上から外れた場合、別リンクへ移った場合、ウィンドウ外へ出た場合、またはリンクを開いた場合はプレビューを消す。

### 10.5 配布ビルド資産

配布ビルドで使用するアプリアイコンは、リポジトリ管理下の `resources/icon.png` を単一のソースとする。

- `electron-builder` の `buildResources` は `resources` ディレクトリを参照する
- macOS / Windows 向けアイコン設定はともに `resources/icon.png` を直接参照する
- アイコン生成のための OS 依存前処理は持たない
- `build/` ディレクトリは配布資産置き場としては使用しない
...
7. ファイルが見つからず親ディレクトリだけは存在する場合、親ディレクトリを開くか利用者へ確認する
8. 失敗時は `AppError` を返す

外部サイト URL のオープン失敗は `LINK_OPEN_FAILED` とする。
`file://` URL の解析失敗は `LINK_INVALID_URL`、対象不存在は `LINK_TARGET_NOT_FOUND`、利用者取消は `LINK_OPEN_CANCELLED`、OS への表示委譲失敗は `LINK_OPEN_FAILED` とする。
...
| リンクオープンエラー | 未許可プロトコル、親フォルダまで解決できない対象不存在、OS 委譲失敗 | 実行画面を維持して通知 |
| リンクオープン取消 | 対象ファイル不存在時の利用者取消 | 実行画面を維持し、エラー扱いしない |
...
- 利用者がリンクオープン確認をキャンセルした場合は、エラーバナーを表示しない
...
- `src/renderer/components/RenderBlocks.tsx`
  本文ブロック描画、手順本文内リンククリック捕捉、リンク hover 捕捉
- `src/renderer/components/ConfirmChecklist.tsx`
  確認項目表示、確認項目内リンク hover 捕捉
...
- 手順本文リンクの hover 開始から約 0.5 秒後に URI プレビューが表示されること
- リンク上からカーソルが外れた時点で URI プレビューが消えること
- セッション JSON バリデーション
- エビデンス Markdown 生成
- フェーズ移動ガード判定
- 未確認項目ありでの強制遷移記録
...
- 存在しない `file://` ファイルリンクのクリックで、親フォルダを開くかの確認が表示され、続行時のみ親フォルダが開くこと
