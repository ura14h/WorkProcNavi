...
- 手順書読込、解析、表示、進捗保存、再開、エビデンス出力の仕様
- 画面責務、状態遷移、エラー処理
- 内部データモデルとファイル形式
- モジュール責務とテスト観点
...
1. Main Process
   ウィンドウ生成、ファイル操作、ZIP 展開、セッション保存、エビデンス出力、カスタムプロトコル提供を担当する。
2. Preload
...
- それ以外の段落、通常リスト、表、引用、コードブロック、画像は本文要素として保持する
...
4. 元手順書を再解析する
5. `currentPhaseId`、`currentStepId`、`checkedItemIds` を再適用する
6. 不整合があれば開始前エラーとする
...
1. セッションに `status = completed` と `completedAt` を設定する
2. エビデンス出力を行う
3. 出力成功時に `.session` を削除する
4. ZIP 展開ディレクトリを削除する
5. 完了画面を表示する
...
appVersion: 0.1.0
manualId: sample-proc-001
sourceFile: sample-proc-001.zip
startedAt: 2026-04-04T09:00:00+09:00
completedAt: 2026-04-04T09:35:00+09:00
phaseCount: 2
stepCount: 5
confirmItemCount: 12
---
...
エビデンスには本文ブロックを再掲しない。
初期版では、実施結果の証跡として確認項目結果とメタ情報の出力を優先する。
...
| execution | すべて完了 | 最終フェーズ完了 | completion |
| execution | 中断保存押下 | 保存成功 | home |
| completion | ホームへ戻る | なし | home |

### 11.2 フェーズ移動ガード

- `次へ` は現在フェーズの確認項目が全件完了した場合のみ活性化する
- 最終フェーズでは `次へ` の代わりに `完了` を表示する
- `戻る` は先頭フェーズでは非活性とする
